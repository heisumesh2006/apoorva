// script.js — robust loader + original animation init
(function () {
  // Utilities -------------------------------------------------------------
  ...
  function ensureDrawSVGPlugin(readyCb) {
    // If DrawSVGPlugin already present, done.
    if (typeof DrawSVGPlugin !== "undefined") {
      console.log("DrawSVGPlugin already available.");
      readyCb();
      return;
    }

    // Try fallback CDN (jsDelivr). You can add more fallbacks here if needed.
    var fallback =
      "https://cdn.jsdelivr.net/npm/gsap@3.12.5/dist/DrawSVGPlugin.min.js";
    console.warn(
      "DrawSVGPlugin not found — attempting to load fallback:",
      fallback
    );
    ...
    loadScript(fallback, function (err) {
      if (err) {
        console.error(
          "Failed to load DrawSVGPlugin from fallback:",
          fallback,
          err
        );
        readyCb(err);
      } else {
        console.log("Loaded DrawSVGPlugin from fallback:", fallback);
        readyCb();
      }
    });
  }

  // Wait until DOM ready (jQuery used in your project)
  ...
  // Main init: ensure plugin then run the animation init
  domReady(function () {
    ensureDrawSVGPlugin(function (err) {
      if (err) {
        console.error(
          "Cannot initialize animations because DrawSVGPlugin couldn't be loaded."
        );
        return;
      }
      ...
      try {
        ...
        console.error("gsap.registerPlugin failed:", e);
        // still try to continue — many errors here are fatal though
      }

      // quick sanity check
      console.log("gsap:", typeof gsap, "DrawSVGPlugin:", typeof DrawSVGPlugin);

      // -------------------------
      // ---- YOUR ANIMATION -----
      // (I kept your original animation logic, but placed it here so it only runs
      //  when DrawSVGPlugin exists)
      // -------------------------

      var strokesLeftBottom = $("#LeftBottomGroup_1_ path[id^=Stroke]")
        .toArray()
        .reverse();
      var strokesLeftTop = $("#LeftTopGroup_1_ path[id^=Stroke]")
        .toArray()
        .reverse();
      ...
      var tl = gsap
        ...
        .fromTo(
          dots,
          { autoAlpha: 0 },
          { duration: 6, autoAlpha: 1, ease: "expo.out" },
          "flower3+=1"
        )
        .fromTo(
          dots,
          { scale: 0, transformOrigin: "50% 50%" },
          { duration: 5, scale: 1, ease: "expo.out" },
          "flower3"
        );
